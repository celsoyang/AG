...
public class Conexao {
    ...
    public static void startConnection(String sgbd, String banco, String local, String porta, String user, String password) {
        
        if(sgbd.equalsIgnoreCase("PostgreSql")){
            connectPostgre(banco,local,porta,user,password);
        } else if(sgbd.equalsIgnoreCase("SqlServer")){
            connectSqlServer(banco,local,porta,user,password);
        }
    }
    
    private static void connectPostgre(String banco, String local, String porta, String user, String password) {
        try {
            Class.forName(StringsUtils.DRIVER_POSTGRE);
            setConention(DriverManager.getConnection("jdbc:postgresql://" + local + ":" + porta + "/" + banco, user, password));
    ...
    private static void connectSqlServer(String banco, String local, String porta, String user, String password) {
    }
